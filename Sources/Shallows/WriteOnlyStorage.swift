public protocol WritableStorageProtocol : StorageDesign {
    ...
    func remove(forKey key: Key, completion: @escaping (ShallowsResult<Void>) -> ())
}
...
public struct WriteOnlyStorage<Key, Value> : WriteOnlyStorageProtocol {
    ...
    private let _remove: (Key, @escaping (ShallowsResult<Void>) -> ()) -> ()

    public init(storageName: String, remove: @escaping (Key, @escaping (ShallowsResult<Void>) -> ()) -> (), set: @escaping (Value, Key, @escaping (ShallowsResult<Void>) -> ()) -> ()) {
        self._set = set
        self._remove = remove
...
extension WriteOnlyStorageProtocol {
    ...
    public func mapKeys<OtherKey>(to type: OtherKey.Type = OtherKey.self,
        ...
        return WriteOnlyStorage<OtherKey, Value>(storageName: storageName, remove: { (key, completion) in
            do {
                let newKey = try transform(key)
                self.remove(forKey: newKey, completion: completion)
            } catch {
    ...
    public func mapValues<OtherValue>(to type: OtherValue.Type = OtherValue.self,
        ...
        return WriteOnlyStorage<Key, OtherValue>(storageName: storageName, remove: self.remove, set: { (value, key, completion) in
            do {
...
extension WriteOnlyStorageProtocol {
    ...
    public func usingUnsupportedTransformation<OtherKey, OtherValue>(_ transformation: (Storage<Key, Value>) -> Storage<OtherKey, OtherValue>) -> WriteOnlyStorage<OtherKey, OtherValue> {
        ...
        }, remove: self.remove, set: self.set)
